...
# Adaptive dropout
DROPOUT_SCALE    = 0.17    # scaling factor (not the max!)
DROPOUT_MAX      = 0.9     # hard ceiling
DROPOUT_EXPONENT = -0.22  # negative = rises as train acc rises

# Dropout smoothing — 1 = instant, higher = more inertia
# e.g. 2 = takes ~2 epochs to reach target, 5 = takes ~5 epochs
DROPOUT_SPRING   = 6

# Dropout update frequency — 1 = every epoch, 2 = every 2 epochs, etc.
UPDATE_EVERY     = 8
...
for epoch in range(1, EPOCHS + 1):
    ...
    # only update dropout every UPDATE_EVERY epochs for extra stability
    if epoch % UPDATE_EVERY == 0:
        target_dropout  = compute_dropout(train_acc)
        current_dropout = current_dropout + (target_dropout - current_dropout) / DROPOUT_SPRING
    ...
    print(f"{epoch:4d} | train: {train_acc*100:.2f}% | "
          ...
          f"Dropout: {current_dropout:.3f} | time: {time.time()-t_start:.2f}s")
