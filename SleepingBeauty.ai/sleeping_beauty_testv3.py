...
Sleeping Beauty v3 (Neural Baker Dream State).
"""
...
# --------------------------------------------------------------------------- #
# Neural Baker (Conditional VAE for Dream Generation)
# --------------------------------------------------------------------------- #
class NeuralBaker(nn.Module):
    def __init__(self, latent_dim=32):
        super().__init__()
        self.latent_dim = latent_dim
        # Encoder: 784 pixels + 10 class labels -> latent space
        ...
        # Decoder: latent space + 10 class labels -> 784 pixels
...
if __name__ == "__main__":
    ...
    # ── Training loop ───────────────────────────────────────────────────────
    print("=" * 80)
    print("TRAINING WITH POSITIONAL ID ENCODING + ROW SAMPLING + BAKER SLEEP")
    print("=" * 80)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # 💤 TRIGGER SLEEPING BEAUTY v3 (Neural Baker Dream State) 💤
        if not is_sleeping and epochs_no_improve >= SLEEP_PATIENCE:
            is_sleeping = True
            sleep_counter = SLEEP_DURATION
            for g in optimizer.param_groups:
                g["lr"] = 0.00005  
            print(f"\n  [💤 ENTERING REM SLEEP] Epoch {epoch} | Baker is generating synthetic dream images...")

        if is_sleeping:
            model.train()
            baker.eval() # Freeze Baker weights, just use it to generate
            
            for _ in range(5):
                # 1. Pick random classes we want the AI to dream about
                y_dream = torch.randint(0, NUM_CLASSES, (BATCH_SIZE,), device=DEVICE)
                y_dream_oh = F.one_hot(y_dream, NUM_CLASSES).float()
                
                # 2. Neural Baker bakes new randomized images based on those classes
                with torch.no_grad():
                    z = torch.randn(BATCH_SIZE, baker.latent_dim, device=DEVICE)
                    X_dream = baker.decode(z, y_dream_oh)
                
                # 3. Feed dream to AI and force it to predict the value we put in
                optimizer.zero_grad(set_to_none=True)
            ...
            if sleep_counter <= 0:
                is_sleeping = False
                epochs_no_improve = 0
                print(f"  [☀️ WAKING UP] Epoch {epoch} | Resuming normal training with smoothed weights.\n")
            
            elapsed = time.time() - t0
            print(f"{epoch:5d} | [💤 Baker Dreaming] consolidation phase... | time: {elapsed:.2f}s")
            continue
